...
function handleData(req, res) {
    ...
    let mousePress = req.body.mousePress;
    let keys = req.body.keys;
    let focus = req.body.focus;

    // TODO: separate the data into likely chunks based on time?
    let dataPoints = [mouse, mousePress, keys, focus];

    let score = dataPoints.reduce(d => calc.generateRSquared(d), 0);
    let maxScore = dataPoints.length;
